...
import pe.edu.upc.trabajofinal.dtos.ProductoCantidadDTO;
...
public class ProductosController {
    ...
    @GetMapping("/pcantidad")
    public List<ProductoCantidadDTO> pcantidad(){
        List<String[]>lista=Ip.pcantidadService();
        List<ProductoCantidadDTO> dtos=new ArrayList<>();
        for(String[] columna:lista) {
            ProductoCantidadDTO dto=new ProductoCantidadDTO();
            dto.setNombreProducto(columna[0]);
            dto.setPcantidad(columna[1]);
            dtos.add(dto);
        }
        return dtos;
    }
